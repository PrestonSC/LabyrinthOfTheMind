...
AttributeComponent::AttributeComponent(int level)
{
	this->level = level;
	this->exp = 0;
	this->expNext = static_cast<int>((50 / 3) * (pow(this->level + 1, 3) - 6 * 
		pow(this->level + 1, 2) + ((this->level + 1) * 17) - 12));
...
void AttributeComponent::updateLevel()
{
	while (this->exp >= this->expNext)
	{
		...
		this->expNext = static_cast<int>((50 / 3) * (pow(this->level, 3) - 6 *
			pow(this->level, 2) + (this->level * 17) - 12));
...
void AttributeComponent::updateStats(const bool reset)
{
	...
	this->wisdom    = this->intelligence * 2 + this->intelligence / 5;
