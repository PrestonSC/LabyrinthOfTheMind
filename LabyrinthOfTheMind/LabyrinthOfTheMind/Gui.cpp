	...
	short unsigned id) : id(id)
{
	...
	this->text.setPosition(
		...
		this->shape.getPosition().y - character_size / 3 + (this->shape.getGlobalBounds().height / 2.f) - this->text.getGlobalBounds().height / 2.f
	);
...
void gui::Button::setButtonState()
{
...
gui::DropDownList::DropDownList(float x, float y, 
	...
	unsigned num_elements, unsigned default_index)
	...
	if (!this->hidden)
	{
		...
		if (this->active)
		{
			this->mousePosGrid.x = (mousePosWindow.x -
				static_cast<int>(this->bounds.getPosition().x)) /
				static_cast<unsigned>(this->gridSize);
			this->mousePosGrid.y = (mousePosWindow.y -
				static_cast<int>(this->bounds.getPosition().y)) /
				static_cast<unsigned>(this->gridSize);
			...
			this->textureRect.left = static_cast<int>(
				this->selector.getPosition().x - this->bounds.getPosition().x
				);
			this->textureRect.top = static_cast<int>(
